...
app.config['MAIL_USERNAME'] = 'motionmartialapps@gmail.com'
app.config['MAIL_PASSWORD'] = 'bauj wnwk nucv zvxn'
app.config["JWT_SECRET_KEY"] = os.getenv('JWT_SECRET_KEY')
app.config['MAIL_DEFAULT_SENDER'] = 'motionmartialapps@gmail.com'
...
mongo.db.users.create_index(
    [("username", TEXT), ("email", TEXT)], default_language='english')
...
def protected():
    ...
    profile_picture_url = url_for(
        'static', filename='uploads/' + user_data.get('photo', 'default.jpg'))
    return jsonify(
        username=user_data['username'],
        email=user_data['email'],
...
def forgot_password():
    ...
    token = create_access_token(identity=str(
        user_data['_id']), expires_delta=False)
...
if __name__ == '__main__':
    app.run(host='192.168.43.213', port=5000, debug=True)
